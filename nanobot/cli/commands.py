...
def gateway(
    ...
    async def on_heartbeat_notify(response: str) -> None:
        ...
        # Filter out <NO NOTIFICATION> lines and send only relevant notifications
        lines = [line for line in response.split("\n") if "<NO NOTIFICATION>" not in line]
        filtered_response = "\n".join(lines).strip()
        if not filtered_response:
            return  # No notifications to send

        await bus.publish_outbound(
            OutboundMessage(channel=channel, chat_id=chat_id, content=filtered_response)
        )
